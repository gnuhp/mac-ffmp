...
AVInputFormat *av_probe_input_format3(AVProbeData *pd, int is_opened, int *score_ret)
{
    ...
    *score_ret= score_max;
    ...
    av_log(NULL, AV_LOG_INFO, "av_probe_input_format3 is: %s  score: %d\n",(fmt==NULL)?"null":fmt->name, score_ret);
    return fmt;
}
...
{
    int score_ret;
    av_log(NULL, AV_LOG_INFO, "av_probe_input_format2 enter \n");

    AVInputFormat *fmt= av_probe_input_format3(pd, is_opened, &score_ret);

    av_log(NULL, AV_LOG_INFO, "av_probe_input_format2 scoremx:%d ret:%d \n", *score_max, score_ret);
...
AVInputFormat *av_probe_input_format(AVProbeData *pd, int is_opened){
    ...
    av_log(NULL, AV_LOG_INFO, "av_probe_input_format enter \n");
    int score=0;
                          ...
                          unsigned int offset, unsigned int max_probe_size)
{
    ...
    av_log(logctx, AV_LOG_DEBUG, "av_probe_input_buffer enter , maxprobesize: %d \n", max_probe_size); 
    if (!max_probe_size) {
    ...
    av_log(logctx, AV_LOG_DEBUG, "av_probe_input_buffer01 %d\n",1); 
    ...
    for(probe_size= PROBE_BUF_MIN; probe_size<=max_probe_size && !*fmt;
        ...
        av_log(logctx, AV_LOG_DEBUG, "probe_size :%d", probe_size); 
        if (probe_size < offset) {

            av_log(logctx, AV_LOG_DEBUG, "probe_size < offset"); 
            continue;
        }
        ...
        av_log(logctx, AV_LOG_INFO, "first 8bytes:%02x %02x %02x %02x  %02x %02x %02x %02x \n",
        ...
        *fmt = av_probe_input_format2(&pd, 1, &score);
        if(*fmt){
            ...
            av_log(NULL, AV_LOG_INFO, "GOT FMT... \n");
        ...
        {
            av_log(logctx, AV_LOG_ERROR, "Format = null, probesize: %d score:%d", probe_size, score); 
        }
    }


    av_log(logctx, AV_LOG_INFO, "av_probe_input_buffer02 %d\n",2); 
    ...
    av_log(logctx, AV_LOG_INFO, "av_probe_input_buffer03 %d\n",3); 

    //20120613:phung: Dont know why it stuck in the below function if i use buffer instead of a file
   //                for now ..  HACK if filename = null skipp this step
    ...
    if (filename != NULL)
    {
        ...
        av_log(NULL, AV_LOG_INFO, "GOT RERTURN\n");
    }
    return ret;
}
...
static int init_input(AVFormatContext *s, const char *filename, AVDictionary **options)
{
    ...
    av_log(NULL, AV_LOG_INFO, "init_input enter 01 filename: %s", filename);

    //20120920: phung For now  skip this purposely
#if 1
    if (s->pb)
    {
        s->flags |= AVFMT_FLAG_CUSTOM_IO;

        av_log(NULL, AV_LOG_INFO, "init_input enter 02");
        if (!s->iformat)
...
#endif 
    av_log(NULL, AV_LOG_INFO, "init_input enter 015");
    if ( (s->iformat && s->iformat->flags & AVFMT_NOFILE) ||
        (!s->iformat && (s->iformat = av_probe_input_format(&pd, 0))))
        return 0;

    av_log(NULL, AV_LOG_INFO, "init_input enter 03");
    ...
    av_log(NULL, AV_LOG_INFO, "init_input enter 04");
    return av_probe_input_buffer(s->pb, &s->iformat, filename, s, 0, 0);
}
...
int avformat_open_input(AVFormatContext **ps, const char *filename, AVInputFormat *fmt, AVDictionary **options)
{
    ...
    av_log(NULL, AV_LOG_INFO, "avformat_open_input enter 01");
    ...
    av_log(NULL, AV_LOG_INFO, "avformat_open_input enter 02");
    if (options)
        av_dict_copy(&tmp, *options, 0);

    av_log(NULL, AV_LOG_INFO, "avformat_open_input enter 03");
    if ((ret = av_opt_set_dict(s, &tmp)) < 0)
        goto fail;

    av_log(NULL, AV_LOG_INFO, "avformat_open_input enter 04");
    if ((ret = init_input(s, filename, &tmp)) < 0)
        goto fail;

    av_log(NULL, AV_LOG_INFO, "avformat_open_input enter 05");
    ...
    av_log(NULL, AV_LOG_INFO, "avformat_open_input enter 06");
    ...
    av_log(NULL, AV_LOG_INFO, "avformat_open_input enter 07");
